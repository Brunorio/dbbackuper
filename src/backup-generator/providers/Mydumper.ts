...
export default class MydumperProviderBackupGenerator extends BackupGenerator {
  ...
  async generate(): Promise<FileGenerateInfo> {
    ...
    execSync(`mydumper --host ${this.credentials.host} --user ${this.credentials.user} --password '${this.credentials.password}' --port ${this.credentials.port} --database ${this.credentials.database} --outputdir ${pathfolder} --compress --triggers --events --routines --verbose 3 --ssl-mode DISABLED`);
